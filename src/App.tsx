...
import { Bell, BriefcaseBusiness, Check, CirclePlus, Menu, X } from 'lucide-react'
...
function App() {
  ...
  return <div className="board-texture min-h-screen bg-board text-paper">
    <header className="sticky top-0 z-30 border-b border-white/10 bg-board/95 backdrop-blur-md"><div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between px-5 lg:px-8"><div className="flex items-center gap-3"><button className="rounded-lg p-2 text-white/70 hover:bg-white/10 lg:hidden" onClick={() => setMobileNav(true)} aria-label="Open menu"><Menu size={21} /></button><div className="flex items-center gap-2.5"><span className="h-3.5 w-3.5 rounded-full bg-coral shadow-[0_2px_5px_rgba(0,0,0,.5)]" /><span className="font-display text-lg font-bold tracking-tight">GigBoard</span></div></div><nav className="hidden items-center gap-8 text-sm text-white/55 lg:flex"><button onClick={() => navigate('browse')} className={view === 'browse' ? 'font-semibold text-paper' : 'hover:text-paper'}>Browse gigs</button><button onClick={() => document.getElementById('how')?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-paper">How it works</button><button onClick={() => navigate('profile')} className="hover:text-paper">Community</button></nav><div className="flex items-center gap-2"><button onClick={() => navigate('orders')} className="hidden items-center gap-2 rounded-lg border border-white/20 px-3 py-2 text-xs font-semibold text-paper transition hover:border-white/50 sm:flex"><BriefcaseBusiness size={15} /> My orders</button><button onClick={() => showNotice('You are all caught up.')} className="rounded-lg border border-white/15 p-2 text-white/70 hover:bg-white/10" aria-label="Notifications"><Bell size={17} /></button><button onClick={() => navigate('auth')} className="hidden rounded-lg border border-white/15 px-3 py-2 text-xs font-semibold text-white/70 transition hover:border-white/40 sm:block">Sign in</button><button onClick={() => navigate('create')} className="hidden items-center gap-2 rounded-lg bg-coral px-4 py-2.5 text-sm font-semibold text-board transition hover:bg-[#e1493a] sm:flex"><CirclePlus size={16} /> Post a gig</button><button onClick={() => navigate('profile')} className="ml-1 flex h-9 w-9 items-center justify-center rounded-lg bg-lav font-mono text-xs font-bold text-board" aria-label="Open profile">IO</button></div></div></header>
    {mobileNav && <div className="fixed inset-0 z-50 bg-board/80 lg:hidden" onClick={() => setMobileNav(false)}><aside className="h-full w-72 border-r border-white/10 bg-board-alt p-5" onClick={(event) => event.stopPropagation()}><div className="mb-10 flex items-center justify-between"><span className="font-display text-xl font-bold">GigBoard</span><button onClick={() => setMobileNav(false)} aria-label="Close menu"><X size={20} /></button></div><nav className="grid gap-2 text-sm"><button onClick={() => navigate('browse')} className="rounded-lg bg-white/10 px-4 py-3 text-left font-semibold">Browse gigs</button><button onClick={() => navigate('orders')} className="rounded-lg px-4 py-3 text-left text-white/60">My orders</button><button onClick={() => navigate('create')} className="rounded-lg px-4 py-3 text-left text-white/60">Post a gig</button><button onClick={() => navigate('profile')} className="rounded-lg px-4 py-3 text-left text-white/60">Community</button><button onClick={() => navigate('auth')} className="rounded-lg px-4 py-3 text-left text-white/60">Sign in</button></nav></aside></div>}
